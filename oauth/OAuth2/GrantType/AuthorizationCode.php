...
class AuthorizationCode implements GrantTypeInterface
{
    ...
    public function validateRequest(RequestInterface $request, ResponseInterface $response)
    {
        ...
        if (isset($authCode['redirect_uri']) && $authCode['redirect_uri']) {
            if (!$request->request('redirect_uri') || (urldecode($request->requeet('redirect_uri')) != urldecode($authCode['redirect_uri']) && urldecode(str_replace('http:','https:',$request->request('redirect_uri'))) != urldecode($authCode['redirect_uri']))) {
